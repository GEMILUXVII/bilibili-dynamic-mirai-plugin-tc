...
val Long.formatZero: String
  get() = if (this in 1..9) "0$this" else this.toString()
val Int.formatZero: String
  get() = this.toLong().formatZero

fun imgApi(imgUrl: String, width: Int, height: Int): String =
        "${imgUrl}@${width}w_${height}h_1e_1c.png"
  ...
  get() =
          when (type) {
            ...
            DYNAMIC_TYPE_ARTICLE ->
                    "https://www.bilibili.com/read/cv${modules.moduleDynamic.major?.article?.id}"
            DYNAMIC_TYPE_AV ->
                    "https://www.bilibili.com/video/${modules.moduleDynamic.major?.archive?.bvid}"
            DYNAMIC_TYPE_MUSIC ->
                    "https://www.bilibili.com/audio/au${modules.moduleDynamic.major?.music?.id}"
            DYNAMIC_TYPE_LIVE ->
                    "https://live.bilibili.com/${modules.moduleDynamic.major?.live?.id}"
            DYNAMIC_TYPE_LIVE_RCMD ->
                    "https://live.bilibili.com/${modules.moduleDynamic.major?.live?.id}"
            DYNAMIC_TYPE_PGC ->
                    "https://www.bilibili.com/bangumi/play/ep${modules.moduleDynamic.major?.pgc?.epid}"
            DYNAMIC_TYPE_PGC_UNION ->
                    "https://www.bilibili.com/bangumi/play/ep${modules.moduleDynamic.major?.pgc?.epid}"
            DYNAMIC_TYPE_UGC_SEASON ->
                    "https://www.bilibili.com/video/av${modules.moduleDynamic.major?.ugcSeason?.aid}"
            DYNAMIC_TYPE_NONE -> ""
          }

fun loadResource(file: String) = BiliBiliDynamic::class.java.getResource(file)?.path!!
// BiliBiliDynamic::class.java.getResource(file)!!.openStream().use { it.readBytes() }

fun loadResourceBytes(path: String) = BiliBiliDynamic.getResourceAsStream(path)!!.readBytes()

val cachePath: Path by lazy { dataFolderPath.resolve("cache") }

fun CacheType.cachePath(): Path {
  return cachePath.resolve(path).apply { if (notExists()) createDirectories() }
}
...
fun CacheType.cacheFile(filePath: String): Path {
  ...
  val file = split.last()
  return cachePath().resolve(path).apply { if (notExists()) createDirectories() }.resolve(file)
}
    ...
    return if (filePath.exists()) {
      ...
      try {
        biliClient.useHttpClient {
          it.get(url).body<ByteArray>().apply { filePath.writeBytes(this) }
        }
      } catch (t: Throwable) {
...
suspend fun getOrDownloadImage(url: String, cacheType: CacheType = CacheType.UNKNOWN) =
        try {
...
suspend fun getOrDownloadImageDefault(
        url: String,
        fallbackUrl: String,
...
suspend fun Contact.sendImage(url: String, cacheType: CacheType = CacheType.UNKNOWN) =
        try {
          getOrDownload(url, cacheType)?.toExternalResource()?.let {
            sendImage(it.toAutoCloseable())
          }
        } catch (e: Exception) {
...
suspend fun Contact.uploadImage(url: String, cacheType: CacheType = CacheType.UNKNOWN) =
        try {
          getOrDownload(url, cacheType)?.toExternalResource()?.let {
            uploadImage(it.toAutoCloseable())
          }
        } catch (e: Exception) {
...
suspend fun List<Contact>.uploadImage(
        url: String,
        cacheType: CacheType = CacheType.UNKNOWN
): String? {
  var sc: String? = null
...
suspend fun List<Contact>.uploadImage(path: Path): String? {
  return try {
    logger.debug("[uploadImage] 读取图片文件: $path")
    val bytes = path.readBytes()
    logger.debug("[uploadImage] 文件读取成功，大小: ${bytes.size} 字节")
    val resource = bytes.toExternalResource().toAutoCloseable()
    uploadImage(resource)
  } catch (e: Exception) {
    logger.error("[uploadImage] 读取或处理图片文件失败: $path", e)
    null
  }
}
...
suspend fun List<Contact>.uploadImage(resource: ExternalResource): String? {
  for (c in this) {
    runCatching {
      logger.debug("[uploadImage] 尝试上传到联系人: ${c.id}")
      val result = c.uploadImage(resource).serializeToMiraiCode()
      logger.debug("[uploadImage] 上传成功: $result")
      result
    }
            .onSuccess {
              return it
            }
            .onFailure { e -> logger.warning("[uploadImage] 上传到 ${c.id} 失败: ${e.message}") }
  }
  logger.warning("[uploadImage] 所有联系人上传均失败")
  return null
}
 ...
 * @author cssxsh
 */
val Contact.delegate
  get() = (if (this is Group) id * -1 else id).toString()
...
inline fun matchUser(user: String, matchSuccess: (uid: Long) -> String?): String? {
  ...
  return if (u.isEmpty()) {
    ...
    buildString {
      appendLine("有多个匹配项：")
      u.forEach { appendLine("${BiliData.dynamic[it.first]?.name}: ${it.second}") }
    }
  }
}
