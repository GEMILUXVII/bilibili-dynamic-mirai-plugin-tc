...
object BiliData : AutoSavePluginData("BiliData") {
  @ValueDescription("数据版本") var dataVersion: Int by value(0)
  ...
  // key: group name
  @ValueDescription("分组") val group: MutableMap<String, Group> by value()

  // key: season id
  @ValueDescription("番剧") val bangumi: MutableMap<Long, Bangumi> by value(mutableMapOf())
}
...
data class SubData(
        ...
        var lastTopDynamic: String = "", // 当前置顶动态的ID
        var lastTopComment: Long = 0, // 当前置顶动态下的置顶评论ID
        var isCheckTop: Boolean = false,
