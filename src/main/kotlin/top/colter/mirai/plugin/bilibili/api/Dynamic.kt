 ...
 * @param offset 动态偏移
 */
suspend fun BiliClient.getUserNewDynamic(
        uid: Long,
...
suspend fun BiliClient.getVideoDetail(id: String): VideoDetail? {
  return getData(VIDEO_DETAIL) {
    if (id.contains("BV")) parameter("bvid", id) else parameter("aid", id.removePrefix("av"))
  }
}

suspend fun BiliClient.getArticleDetailOld(id: String): ArticleDetail? {
  return getData(ARTICLE_DETAIL) {
    if (id.startsWith("cv")) parameter("id", id.removePrefix("cv")) else parameter("id", id)
  }
}
...
suspend fun BiliClient.getArticleList(ids: List<String>): Map<String, ArticleDetail>? {
  return getData(ARTICLE_LIST) { parameter("ids", ids.joinToString(",")) }
}

/**
 * 获取动态评论
 * @param oid 评论区ID
 * @param type 评论类型 (1:视频, 11:图片, 12:专栏, 17:动态)
 */
suspend fun BiliClient.getComment(oid: String, type: Int = 17): Comment? {
  return getData(COMMENT_REPLY) {
    parameter("oid", oid)
    parameter("type", type)
    parameter("ps", 1)
    parameter("root", -1)
