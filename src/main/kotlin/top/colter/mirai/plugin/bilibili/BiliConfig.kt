...
object BiliConfig : ReadOnlyPluginConfig("BiliConfig") {
  ...
  // val help: String by
  // value("https://github.com/Colter23/bilibili-dynamic-mirai-plugin#BiliConfig.yml")

  @ValueDescription("管理员") val admin: Long by value(0L)
  ...
  @ValueDescription(
        ...
        检测配置:
          ...
          topCommentInterval: 置顶评论检测间隔(推荐 3-10，更快响应) 单位秒
...
data class CheckConfig(
        ...
        var topCommentInterval: Int = 10, // 置顶评论检测间隔，默认10秒(过低会触发B站风控)
...
data class TemplateConfig(
        ...
        val dynamicPush: MutableMap<String, String> =
                mutableMapOf(
                        "DrawOnly" to "{draw}",
                        "TextOnly" to "{name}@{type}\n{link}\n{content}\n{images}",
                        "OneMsg" to "{draw}\n{name}@{type}\n{link}",
                        "OneMsgWithImages" to "{draw}\n{name}@{type}\n{link}\n{images}",
                        "TwoMsg" to "{draw}\r{name}@{uid}@{type}\n{time}\n{link}",
                        "ForwardMsg" to
                                "{draw}{>>}作者：{name}\nUID：{uid}\n时间：{time}\n类型：{type}\n链接：{links}\r{content}\r{images}{<<}",
                ),
        val livePush: MutableMap<String, String> =
                mutableMapOf(
                        "DrawOnly" to "{draw}",
                        "TextOnly" to "{name}@直播\n{link}\n标题: {title}",
                        "OneMsg" to "{draw}\n{name}@直播\n{link}",
        ...
        val liveClose: MutableMap<String, String> =
                mutableMapOf(
