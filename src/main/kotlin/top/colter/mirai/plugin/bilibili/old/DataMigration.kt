      ...
      BiliSubscribeData.dynamic.forEach { (t, u) ->
        if (!BiliData.dynamic.containsKey(t) || t == 0L) {
          BiliData.dynamic[t] =
                  SubData(
                          name = u.name,
                          color = if (u.color == "#d3edfa") null else u.color,
                          last = u.last,
                          lastLive = u.lastLive,
                          lastTopDynamic = "", // 新增字段，默认为空
                          lastTopComment = 0, // 新增字段，默认为0
                          isCheckTop = false, // 新增字段，默认关闭
                          contacts = u.contacts.keys.toMutableSet(),
                          banList = u.banList
                  )
...
fun migrationConfig() {
  if (BiliBiliDynamic.configFolder.resolve("BiliPluginConfig.yml").exists()) {
    ...
    if (!BiliPluginConfig.migrated) {
      ...
      BiliConfig.enableConfig.translateEnable =
              BiliPluginConfig.baiduTranslate["enable"].toBoolean()
...
fun updateData() {
  if (BiliData.dataVersion == 0) {
    ...
    BiliData.livePushTemplate.forEach {
      val s = it.value.map { findContactAll(it.toLong())?.delegate }.filterNotNull().toSet()
      it.value.clear()
      it.value.addAll(s)
    }
    BiliData.atAll.apply {
      val m = map { (findContactAll(it.key.toLong())?.delegate ?: it.key) to it.value }.toMap()
      clear()
      putAll(m)
    }
