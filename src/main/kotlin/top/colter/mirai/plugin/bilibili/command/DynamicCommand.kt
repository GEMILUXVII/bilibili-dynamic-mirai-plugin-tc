...
import top.colter.mirai.plugin.bilibili.tasker.TopCommentCheckTasker
...
object DynamicCommand :
  ...
  private val showLoadingMessage
    get() = BiliConfig.enableConfig.showLoadingMessage

  @SubCommand("h", "help", "帮助", "menu")
  suspend fun CommandSender.help() {
    loadResourceBytes("image/HELP.png")
            .toExternalResource()
            .toAutoCloseable()
  ...
  suspend fun CommandSender.color(user: String, color: String) {
    matchUser(user) { DynamicService.setColor(it, color) }?.let {
      sendMessage(it)
    ...
    if (checkPerm(target)) {
      if (pgcRegex.matches(id)) {
        sendMessage(PgcService.followPgc(id, target.subject))
              ...
              try {
                DynamicService.addSubscribe(
                                id.toLong(),
                                target.subject,
                                this.subject?.delegate == target.subject
                        )
                        .let {
                          sendMessage(it)
    ...
    if (checkPerm(target)) {
      if (pgcRegex.matches(id)) {
        sendMessage(PgcService.delPgc(id, target.subject))
              ...
              matchUser(id) {
                DynamicService.removeSubscribe(
                        it,
                        target.subject,
                        this.subject?.delegate == target.subject
                )
  ...
  suspend fun CommandSender.listUser(user: String = "") {
    if (admin == Contact().id || admin == this.user?.id || this.user == null)
            if (user.isEmpty()) {
              ...
              matchUser(user) { DynamicService.listUser(it) }?.let { sendMessage(it) }
            }
    else sendMessage("仅bot管理员可获取")
  }
  ...
  suspend fun CommandSender.filterList(
          uid: Long = 0L,
          target: GroupOrContact = GroupOrContact(Contact())
  ) {
    if (checkPerm(target)) {
  ...
  suspend fun CommandSenderOnMessage<*>.login() {
    val subject = Contact()
    if (BiliConfig.admin == subject.id || BiliConfig.admin == user?.id) LoginService.login(subject)
  ...
  suspend fun CommandSender.atall(
    ...
    if (checkPerm(target)) {
      matchUser(user) { AtAllService.addAtAll(type, it, target) }?.let { sendMessage(it) }
    }
  }
  ...
  suspend fun CommandSender.delAtall(
    ...
    if (checkPerm(target)) {
      matchUser(user) { AtAllService.delAtAll(type, it, target.subject) }?.let { sendMessage(it) }
    }
  }

  @SubCommand("listAtall", "laa", "at全体列表")
  suspend fun CommandSender.listAtall(
          user: String = "0",
          target: GroupOrContact = GroupOrContact(Contact())
  ) {
    if (checkPerm(target)) {
      matchUser(user) { AtAllService.listAtAll(it, target.subject) }?.let { sendMessage(it) }
    }
  }
  ...
  suspend fun CommandSenderOnMessage<*>.live() {
    ...
    detail?.let { d ->
      BiliBiliDynamic.liveChannel.send(LiveDetail(d.rooms.first(), subject.delegate))
    }
  }
  ...
  suspend fun CommandSenderOnMessage<*>.clear() {
    if (BiliConfig.admin == user!!.id) {
      ...
      try {
        ...
        if (msg.content == "确定") {
          map.forEach { (c, u) -> u.forEach { DynamicService.removeSubscribe(it, c) } }
          sendMessage("删除成功")
  ...
  @SubCommand("topComment", "tc", "置顶评论")
  suspend fun CommandSender.topComment(
          user: String,
          target: GroupOrContact = GroupOrContact(Contact())
  ) {
    if (checkPerm(target)) {
      matchUser(user) {
        val subData = dynamic[it]
        if (subData != null) {
          subData.isCheckTop = !subData.isCheckTop
          if (subData.isCheckTop) {
            "已开启 ${subData.name} 的置顶评论检测"
          } else {
            "已关闭 ${subData.name} 的置顶评论检测"
          }
        } else {
          "未订阅该用户"
        }
  ...
  @SubCommand("fetchTopComment", "ftc", "拉取置顶评论")
  suspend fun CommandSender.fetchTopComment(
          user: String,
          target: GroupOrContact = GroupOrContact(Contact())
  ) {
    if (checkPerm(target)) {
      matchUser(user) { uid -> TopCommentCheckTasker.fetchTopComment(uid, target.subject) }?.let {
        sendMessage(it)
      }
    }
  }
  ...
  suspend fun CommandSender.checkPerm(target: GroupOrContact): Boolean {
    ...
    if (target.group == null &&
                    !hasPermission(crossContact) &&
                    (target.contact?.delegate ?: "0") != Contact().delegate
    ) {
...
fun CommandSender.Contact(): Contact =
        subject ?: throw CommandArgumentParserException("无法从当前环境获取联系人")
